...
#echo `date` " running on $1 ..." >> /tmp/client_log
...
export UUID=`echo $1 | sed -e "s/\/dev\///g;s/\/.*$//g"`
...
then
	...
	echo LD_LIBRARY_PATH=$BASE_PATH/lib $BASE_PATH/bin/client "$UUID $LPORT $REMOTE_PORT TCP" > /tmp/client_$LPORT
	LD_LIBRARY_PATH=$BASE_PATH/lib $BASE_PATH/bin/client $UUID $LPORT $REMOTE_PORT TCP >>/tmp/client_$LPORT 2>&1 &
	...
	if [ "$2" = "kill" ]
	then
		echo "killing $UUID client (PID $PID) ..."
		kill -TERM $PID
		exit 0
	fi
